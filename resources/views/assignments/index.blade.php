...
@section('content')
  ...
  <div class="flex items-start justify-between">
    <div>
      ...
      <p class="text-sm text-muted mt-1">{{ $assignments->total() }} assignments</p>
    </div>
    @can('assignments.create')
      <a href="{{ route('assignments.create') }}" class="bg-brand hover:bg-brand-d text-white text-sm font-medium rounded-md px-4 py-2 shadow-sm">+ New Assignment</a>
    @endcan
  </div>

  <form method="GET" class="flex flex-wrap gap-2">
    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Search crew name or code..."
          ...
          @forelse($assignments as $assignment)
            <tr class="border-t border-line hover:bg-slate-50">
              ...
              <td class="px-3 py-3">
                ...
                <div class="text-[11px] text-muted">{{ $assignment->employee_id ?: 'not an employee yet' }}</div>
              </td>
              ...
              <td class="px-5 py-3 text-right">
                <a href="{{ route('assignments.show', $assignment) }}" class="text-brand hover:text-brand-d font-medium">Open</a>
              </td>
            </tr>
          @empty
            <tr><td colspan="8" class="px-5 py-10 text-center text-muted">No assignments yet.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
